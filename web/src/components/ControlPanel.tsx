...
const FALLBACK_RULES: ContractRules = {
  maxDailyLimit: 50,
  spentToday: 0,
...
const FALLBACK_LOGS: TerminalLog[] = [
  {
    id: "local-preview-1",
    timestamp: new Date().toISOString(),
    type: "info",
    emitter: "State",
    message: "Demo backend is offline. Showing local preview state in the dashboard.",
  },
];
...
export default function ControlPanel() {
  ...
  // Session signer private key state
  const [sessionKey, setSessionKey] = useState<string>("0xbf4d28f0de9ca2b8da46e8eb497fae017686901f2ab4ce9b1d3f5a7c9e0b123");
  const [showCopied, setShowCopied] = useState(false);

  // Poll intervals
  const pollIntervalRef = useRef<NodeJS.Timeout | null>(null);

  const appendLocalLog = (message: string, type: TerminalLog["type"] = "info", emitter: TerminalLog["emitter"] = "State") => {
  ...
  const parseJsonResponse = async <T,>(res: Response, endpoint: string): Promise<T> => {
    const contentType = res.headers.get("content-type") ?? "";
    if (!res.ok) {
      throw new Error(`${endpoint} returned ${res.status}`);
    }
    if (!contentType.includes("application/json")) {
      throw new Error(`${endpoint} returned non-JSON content`);
    }
  ...
  const fetchRules = async () => {
    try {
      ...
      const data = await parseJsonResponse<ContractRules>(res, "/api/rules");
      setRules(data);
      setMaxLimitInput(data.maxDailyLimit.toString());
      setBackendOnline(true);
    } catch (e) {
      setBackendOnline(false);
      setRules(prev => prev ?? FALLBACK_RULES);
      setMaxLimitInput((prev => (prev === "50" ? FALLBACK_RULES.maxDailyLimit.toString() : prev)));
    }
  };
  ...
  const fetchLogs = async () => {
    try {
      ...
      const data = await parseJsonResponse<TerminalLog[]>(res, "/api/logs");
      setLogs(data);
      setBackendOnline(true);
    } catch (e) {
      setBackendOnline(false);
  ...
  const handleGenerateKey = () => {
    ...
    const log: TerminalLog = {
      ...
      type: "info",
      emitter: "State",
      message: `Generated owner-issued session signer secret: ${hex.slice(0, 10)}...${hex.slice(-8)}. Demo agent can now sign within delegated limits.`
    };
    setLogs(prev => [log, ...prev]);
  };
  ...
  const handleUpdateRules = async (updatedLimit?: number, toggledAddress?: string) => {
    ...
    try {
      ...
      if (!backendOnline) {
        const localRules = rules ?? FALLBACK_RULES;
        let nextWhitelist = { ...localRules.whitelistedProtocols };
        if (toggledAddress) {
          nextWhitelist[toggledAddress] = {
            ...localRules.whitelistedProtocols[toggledAddress],
            isWhitelisted: !localRules.whitelistedProtocols[toggledAddress].isWhitelisted
          };
        }
        const nextRules = {
          ...localRules,
        ...
        setRules(nextRules);
        setMaxLimitInput(nextRules.maxDailyLimit.toString());
        appendLocalLog("Demo backend offline. Guardrail changes are being previewed locally only.", "warning", "State");
        return;
      }
      ...
      const data = await parseJsonResponse<{ success: boolean; rules: ContractRules }>(res, "/api/rules");
      if (data.success) {
        setRules(data.rules);
  ...
  const handleResetSpent = async () => {
    try {
      if (!backendOnline) {
        const localRules = rules ?? FALLBACK_RULES;
        setRules({ ...localRules, spentToday: 0 });
        appendLocalLog("Demo backend offline. Daily spend counter reset locally only.", "info", "State");
        return;
      }

      const res = await fetch("/api/rules/reset-spent", { method: "POST" });
      const data = await parseJsonResponse<{ success: boolean; spentToday: number }>(res, "/api/rules/reset-spent");
      if (data.success) {
        if (rules) {
        ...
        setLogs([
          {
            ...
            type: "info",
            emitter: "State",
            message: "Local preview logs cleared.",
          },
        ]);
        return;
      }
  ...
  const executeSimulation = async (command: string) => {
    ...
    try {
      if (!backendOnline) {
        appendLocalLog(
          `Demo backend offline. Cannot execute "${command}" until agent-backend is running on port 3001.`,
          "warning",
          "AI Agent"
        );
        setLastActionStatus("OFFLINE");
        return;
      }
      ...
      const data = await parseJsonResponse<SimulationResult>(res, "/api/trigger-ai");
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="mb-4">
            ...
            <p className="text-xs text-[#888] leading-relaxed mt-2">
              Phylax separates AI authority through delegated session signers and enforces policy directly on-chain through <span className="text-cyan-400 font-mono">Programmable Guardrails</span>.
            </p>
            {backendOnline === false && (
              <div className="mt-3 border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-[11px] font-mono text-amber-200">
                Demo backend offline. The dashboard is showing local preview state only. Run <code>pnpm dev:backend</code> to enable live simulator APIs.
              </div>
            )}
          </div>
          ...
          <div className="bg-[#080808] border border-slate-900 p-5 rounded-none relative">
            ...
            <div className="space-y-5">
              ...
              <div className="bg-[#0C0C0C] p-3 border border-slate-900 font-mono text-xs flex items-center justify-between">
                <div>
                  <span className="text-[#555] block text-[9px] uppercase tracking-wider">ACCUMULATED SPENT TODAY</span>
          ...
          <div className="bg-[#080808] border border-slate-900 p-5 rounded-none relative">
            ...
            <p className="text-[11px] text-[#555] mb-3 leading-relaxed font-mono">
              This is the delegated private key revealed to the AI environment. It is not the owner key and should be shown once during setup.
            </p>
            ...
            <div className="bg-[#050505] p-3 rounded-none border border-slate-900 flex items-center justify-between font-mono text-xs">
              ...
              <button
                ...
                title="Copy session signer private key"
              >
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-[#080808] border border-slate-900 p-5 rounded-none relative">
            ...
            <p className="text-xs text-[#888] mb-4 leading-relaxed font-sans">
              Test dynamic policies by dispatching simulated normal or malicious operations. The connected demo backend parses prompts and attempts execution through the delegated session signer.
            </p>
            ...
            <form onSubmit={handleRunCustom} className="space-y-3">
              ...
              <div className="flex items-center justify-between text-[10px] font-mono text-[#555] px-1">
                <span>DEMO AGENT BACKEND PARSER ACTIVE</span>
                {lastActionStatus && (
                  <span className={`font-bold px-1.5 py-0.5 rounded-none text-[9px] uppercase border ${
                    lastActionStatus === "SUCCESS" 
                      ...
                      : "bg-[#1A0808] text-rose-400 border-red-900"
                  }`}>
