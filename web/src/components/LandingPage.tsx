...
export default function LandingPage({ onNavigateToDashboard, onNavigateToDocs }: LandingPageProps) {
  ...
  const faqs = [
    {
      ...
      a: "A Session Key in the Phylax MVP is a delegated AI session signer created by the owner in the web dashboard. Its public address is registered on-chain, while the private key is revealed once and passed to the AI environment. The owner EOA private key is never shared with the agent."
    },
    {
      q: "How do Programmable On-Chain Guardrails protect me?",
      a: "Permissions are enforced directly on-chain inside the guardrail module. The contract checks whether the target protocol is whitelisted, whether spending stays within the configured limit, and whether the delegated session is still active. If any rule is broken, the transaction reverts before capital can move."
    },
    {
      q: "Are the AI transaction operations truly gasless?",
      a: "For the intended MVP flow, yes. ERC-4337 user operations are sponsored through a custom Phylax paymaster, with gas settlement sourced from an owner-managed Gas Tank instead of the AI smart account's working USDC balance."
    },
    {
      q: "How does the AI transform English instructions into on-chain calls?",
      a: "Phylax is model-agnostic. Your AI system decides what it wants to do, then uses the SDK to turn that intent into a guarded ERC-4337 user operation. In this repository, the agent-backend is only a demo app that simulates how an external AI agent could integrate with the SDK."
    },
    {
      q: "What resources can the master owner override?",
      a: "The owner retains custody of the smart account configuration. They can rotate session signers, update protocol whitelists, change spend limits, top up or pause gas coverage, and revoke AI execution rights without handing the owner wallet key to the agent."
    }
  ];
  ...
  const techStack = [
    ...
    },
    {
      ...
      desc: "The execution model that lets delegated AI session signers submit UserOperations through smart accounts with policy enforcement.",
    ...
    },
    {
      ...
      desc: "Owner-issued session signer secrets let AI systems execute within bounded scope without ever receiving the owner EOA key.",
    ...
    },
    {
      ...
      desc: "Bundler infrastructure relays ERC-4337 UserOperations after Phylax assembles and signs them through delegated AI sessions.",
    ...
    <div className="text-[#E0E0E0] max-w-5xl mx-auto px-4 py-8 sm:py-16 space-y-24 relative overflow-hidden">
      ...
      >
        ...
        >
          ...
          OWNER-CONTROLLED AI SESSION SECURITY
        </motion.div>
        ...
        >
          Phylax is an ERC-4337 security layer for AI agents on Arbitrum Sepolia. Owners create bounded session signers for each agent, while on-chain guardrails and paymaster-backed gas coverage keep execution separate from the owner's main wallet authority and the agent's working capital.
        </motion.p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative font-mono">
          ...
          >
            ...
            <p className="text-[11px] text-[#888] leading-normal">
              An external AI agent decides on an action such as swapping, lending, or rebalancing through its own off-chain logic.
            </p>
          </motion.div>
          ...
          >
            ...
            <p className="text-[11px] text-[#888] leading-normal">
              The agent signs the UserOperation with an owner-issued session signer secret. The owner EOA private key never leaves the dashboard setup flow.
            </p>
          </motion.div>
          ...
          >
            ...
            <p className="text-[11px] text-gray-200 leading-normal">
              Phylax contracts enforce expiry windows, spend limits, and protocol whitelists directly on-chain before any external call is executed.
            </p>
          </motion.div>
          ...
          >
            ...
            <p className="text-[11px] text-[#888] leading-normal">
              Allowed actions settle through the bundler and Phylax paymaster flow, while blocked actions revert before the agent can drain funds.
            </p>
          </motion.div>
        </div>
      </motion.div>
      ...
      >
        <div className="text-left space-y-2">
          ...
          <p className="text-[#888] text-xs max-w-xl">
            The current MVP stack combines delegated AI execution, on-chain policy enforcement, and ERC-4337 transaction sponsorship.
          </p>
        </div>
      ...
      >
        <div className="text-left space-y-2">
          ...
          <p className="text-[#888] text-xs max-w-xl">
            Product and integration notes for the current Phylax MVP direction.
          </p>
        </div>
      ...
      >
        ...
        <div className="flex justify-between items-center text-[#555] mb-2">
          ...
          <span className="text-[9px] uppercase tracking-tighter">MVP Preview API</span>
        </div>
        <div className="text-[#888] scale-[0.98] origin-left">
          <span className="text-cyan-400">import</span> {"{"} PhylaxClient {"}"} <span className="text-cyan-400">from</span> <span className="text-orange-300 font-bold">"@phylax/sdk"</span>;
          <br />
          <span className="text-purple-400">const</span> secureClient = <span className="text-cyan-400">await</span> PhylaxClient.<span className="text-purple-400">fromEnv</span>(config);
          <br />
          <span className="text-[#555]">// Ready to submit guarded ERC-4337 actions</span>
        </div>
      </motion.div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="space-y-4">
            ...
            <p className="text-[11px] text-[#555] uppercase tracking-wider font-mono">
              Smart-account guardrails for owner-bounded AI execution on-chain.
            </p>
          </div>
          ...
          <div className="space-y-4 font-mono text-xs">
            ...
            <div className="space-y-2 text-[#888]">
              ...
              <div className="flex items-center gap-2 text-[10px]">
                ...
                <span className="text-[#00FF41]">ARBITRUM SEPOLIA MVP STATUS</span>
              </div>
            </div>
          </div>
        ...
        <div className="border-t border-slate-900 pt-6 flex flex-col md:flex-row justify-between items-center text-[10px] font-mono text-[#555] uppercase tracking-wider gap-4">
          <span>PHYLAX MVP REPOSITORY PREVIEW</span>
          <span>2026 PHYLAX PROTOCOL CONCEPT</span>
        </div>
      </motion.footer>
