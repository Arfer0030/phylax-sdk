  ...
  return (
    <section id="built-for">
      <div className="mx-auto grid w-full max-w-7xl gap-12 px-6 py-24 sm:px-10 lg:grid-cols-[0.9fr_1.1fr] lg:px-12">
        ...
        <StaggerGroup className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" delayChildren={0.08} stagger={0.06}>
          {operatingProfiles.map((profile) => (
            <StaggerItem key={profile.name} y={30}>
              <div className="group flex min-h-[184px] flex-col justify-between border border-white/6 bg-white/[0.03] p-6 transition hover:border-white hover:bg-white">
                <div className="flex justify-end text-zinc-400 transition group-hover:text-black">
                  <ProfileGlyph kind={profile.icon} />
                </div>
                <span className="max-w-[13ch] text-[1.72rem] font-bold leading-[1.08] tracking-[-0.05em] text-white transition group-hover:text-black sm:text-[1.8rem]">
                  {profile.name}
                </span>
              </div>
            </StaggerItem>
